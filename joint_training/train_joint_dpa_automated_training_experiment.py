...
def main():
    ...
    include_pen_e = bool(args.include_pen_e)
    lam = args.lam
    lr = args.lr
    ...
    save_dir = f"{settings['output_dir']}_{latent_dim}_{num_layers}_{hidden_dim}_{noise_dim_dec}_{in_dim_lm}_{noise_dim_lm}_{num_layers_lm}_{hidden_dim_lm}_encoderis{encoder}_lambda{lam}_alpha{args.alpha}_bs{batch_size}_bnis{bn}_lr{lr}_pene{include_pen_e}/"
    ...
    for epoch_idx in range(0, training_epochs):
        ...
        for batch_idx, train_batch in enumerate(train_loader):
            ...
            model_pred.eval()  # if it has dropout/bn, makes it deterministic
            for p in model_pred.parameters():
            ...
            if encoder == "learnable":
                if include_pen_e:
                    ...
                    loss_pre = loss_rec + penalty_e #+ penalty_gen
                else:
                    loss_pre = loss_rec

            else:
                loss_pre = loss_rec #+ penalty_gen
                
            # 4
            loss_pred, s1_pred, s2_pred = energy_loss_two_sample(y, gen1, gen2, verbose=True, beta=beta)


            # advanced: dynamic gamma 
            #gamma = 0.01 * (loss_rec.detach() / (loss_lat.detach() + 1e-8))

            loss_lat, _, _  = energy_loss_two_sample(e, z1, z2, beta=beta)  # latent matching
            loss = loss_pre + args.alpha * loss_pred + 0.0001 * loss_lat
        ...
        if (epoch_idx == 0 or (epoch_idx + 1) % plot_every_epoch == 0):
            ...
            if batch_idx_test == 499:
                ...
                y_te_da = ut.torch_to_dataarray(y_te_from_restored, coords_ds=ds_test, name="Temperature")
                reconstructed_da_11 = ut.torch_to_dataarray(gen1_te_from_restored, coords_ds=ds_test, name="Temperature")
                reconstructed_da_12 = ut.torch_to_dataarray(gen2_te_from_restored, coords_ds=ds_test, name="Temperature")
                reconstructed_da_21 = ut.torch_to_dataarray(gen3_te_from_restored, coords_ds=ds_test, name="Temperature")
