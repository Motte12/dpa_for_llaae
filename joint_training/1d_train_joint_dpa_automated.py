...
def main():
    ...
    save_dir = f"{settings['output_dir']}1d_{latent_dim}_{num_layers}_{hidden_dim}_{noise_dim_dec}_{in_dim_lm}_{noise_dim_lm}_{num_layers_lm}_{hidden_dim_lm}_encoderis{encoder}_lambda{lam}_bs{batch_size}_bnis{bn}/"
    ...
    # split 
    ## train
    X_torch = torch.from_numpy(predictors_combined_le)[:90*4769, :].to(dtype=torch.float32)
    y_torch = torch.from_numpy(trefht_le_ger_mean.values)[:90*4769].to(dtype=torch.float32)
    
    ## validation
    X_val_torch = torch.from_numpy(predictors_combined_le)[90*4769:, :].to(dtype=torch.float32)
    y_val_torch = torch.from_numpy(trefht_le_ger_mean.values)[90*4769:].to(dtype=torch.float32)
    ...
    x_tr_reduced_pre, mask_x_tr = ut.remove_nan_columns(x_tr)
    x_tr_reduced = x_tr_reduced_pre.to(dtype=torch.float32)
    x_te_reduced_pre, mask_x_te = ut.remove_nan_columns(x_te)
    x_te_reduced = x_te_reduced_pre.to(dtype=torch.float32)
    ##########################

    # create data loader Temperature
    train_dataset = TensorDataset(X_torch,x_tr_reduced, y_torch)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False)
    print(f"Number of batches: {len(train_loader)}")
    print(train_loader)
    
    # create test loader Temperature
    test_dataset = TensorDataset(X_val_torch, x_te_reduced, y_val_torch)
    ...
    for epoch_idx in range(0, training_epochs):
        ...
        for batch_idx, train_batch in enumerate(train_loader):
            ...
            loss_rec, s1_rec, s2_rec = energy_loss_two_sample(y_out, rec1, rec2, verbose=True, beta=beta)
            loss = loss_rec
            ...
            #else:
            loss_pred, s1_pred, s2_pred = energy_loss_two_sample(y_out, gen1, gen2, verbose=True, beta=beta)
        ...
        with torch.no_grad():
            for batch_idx_test, test_batch in enumerate(test_loader_in):
                ...
                y_te_out = test_batch[2].to(device)
                ...
                loss_te_rec_pre = energy_loss_two_sample(y_te_out, rec1_te, rec2_te, beta=beta)
                loss_te_pred_pre = energy_loss_two_sample(y_te_out, gen1_te, gen2_te, beta=beta)
