...
#SBATCH --array=0-1296%50   # 864 configs total; run max 52 at a time
#SBATCH --output=/work/fl53wumy-dpa_data/fl53wumy-llaae_data_new_22092025-1763346001/fl53wumy-llaae_data_new-1758244802/fl53wumy-llaae_data_new-1748049607/dpa_output/dpa_tuning4_starting16122025_v4_data/out/slurm-%A_%a.out
#SBATCH --error=/work/fl53wumy-dpa_data/fl53wumy-llaae_data_new_22092025-1763346001/fl53wumy-llaae_data_new-1758244802/fl53wumy-llaae_data_new-1748049607/dpa_output/dpa_tuning4_starting16122025_v4_data/err/slurm-%A_%a.err
...
STATUS_DIR=/work/fl53wumy-dpa_data/fl53wumy-llaae_data_new_22092025-1763346001/fl53wumy-llaae_data_new-1758244802/fl53wumy-llaae_data_new-1748049607/dpa_output/dpa_tuning4_starting16122025_v4_data/status
mkdir -p "$STATUS_DIR"
          ...
          for hdl in "${hidden_dim_lm[@]}"; do
            for ndl in "${noise_dim_lm[@]}"; do
              for lamb in "${lambda[@]}"; do
                for al in "${alpha[@]}"; do
                  configs+=("$ld,$enc,$hdn,$nln,$ndd,$hdl,$ndl,$lamb,$al")
                done  
              done
            done
          done
...
echo "TASK ${SLURM_ARRAY_TASK_ID} → latent_dim=$ld, encoder=$enc, hidden_dim_NN=$hdn, num_layers_NN=$nln, noise_dim_dec=$ndd, hidden_dim_lm=$hdl, noise_dim_lm=$ndl, lambda=$lambd, alpha=$alph"
...
~/.conda/envs/dpa/bin/python ../train_joint_dpa_automated.py \
    --settings_file dpa_tune4_settings.json \
    --epochs 100 \
    --encoder "$enc" \
    --in_dim 648 \
    --latent_dim "$ld" \
