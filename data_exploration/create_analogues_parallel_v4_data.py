...
def main():
    ...
    parser.add_argument("--settings_file_path", type=str, default = "../joint_training/v4_dpa_train_settings.json")
    ...
    print(z500_le.pseudo_pcs)
    #uncomment:
    predictors_combined_le_pre = z500_le.pseudo_pcs.values
    train_predictors, _, _ = ut.standardize_numpy(predictors_combined_le_pre[:90*4769, :])
    #predictors_combined_le = #create xarray#torch.from_numpy(train_predictors)
    
    # create xarray
    ds_ref = z500_le.pseudo_pcs.isel(time=slice(0,90*4769))
    
    predictors_combined_le = xr.DataArray(
        train_predictors,  # or np.zeros(ref.shape)
        dims=ds_ref.dims,
        coords=ds_ref.coords,
        name="pseudo_pcs",
        )
    print(predictors_combined_le)
    ...
    z500_eth = xr.open_dataset(settings['dataset_z500_eth_test'])
    print(z500_eth.pseudo_pcs)
    predictors_combined_eth_pre = z500_eth.pseudo_pcs.values
    test_predictors, _, _ = ut.standardize_numpy(predictors_combined_eth_pre)
    
    #predictors_combined_eth = torch.from_numpy(test_predictors)
    ds_ref_eth = z500_eth.pseudo_pcs
    predictors_combined_eth = xr.DataArray(
        test_predictors,  # or np.zeros(ref.shape)
        dims=ds_ref_eth.dims,
        coords=ds_ref_eth.coords,
        name="pseudo_pcs",
        )
    print(predictors_combined_eth)
    ...
    # ----------------------------------------------------
        # ----------------------------------------------------
    # Helper: work for a single ETH timestep
    # ----------------------------------------------------
    def compute_analogues_for_index(i):
        print(f"Creating analogues for index {i}")
        ...
        # 4) Find indices of n_analogues smallest distances
        ...
        # 5) Corresponding LE times and temperature fields
        ...
        # Return data for this i
        # keep your transpose so output stays the same as before
        return i, ana_temp_fields.values.transpose(2, 1, 0), ana_times.values

    # ----------------------------------------------------
    # Run in parallel over time indices
    # ----------------------------------------------------
    # change 30 -> n_eth if you want all timesteps
    indices = range(n_eth)

    # choose how many threads to use (e.g. match --cpus-per-task in SLURM)
    n_jobs = 8  # or int(os.getenv("SLURM_CPUS_PER_TASK", 8))

    results = Parallel(n_jobs=n_jobs, backend="threading")(
        delayed(compute_analogues_for_index)(i) for i in indices
    )

    # ----------------------------------------------------
    # Fill the big arrays from results
    # ----------------------------------------------------
    for i, temps_i, times_i in results:
        analogue_temps_all[i, :, :, :] = temps_i
        analogue_times_all[i, :] = times_i
        if i % 100 == 0:
    ...
    print(ds_ens)
    ds_ens.assign_coords(time=np.arange(ds_ens.sizes["time"]))
    ds_ens.to_netcdf(f"{args.anal_ens_save_path}analogue_ensemble_{no_pcs}pcs_{n_analogues}analogues_{n_ens}analoguemembers_v4_data_complete.nc")
    print(ds_ens)
